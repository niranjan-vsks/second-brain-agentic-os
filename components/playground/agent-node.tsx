...
export const AgentNode = memo(function AgentNode({ data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`node-face press w-[196px] rounded-xl border p-3 transition-[box-shadow,transform,opacity] duration-200 ${glowClass(
        d.status,
        d.paused,
...
export const GroupNode = memo(function GroupNode({ data, width, height }: NodeProps) {
  ...
  return (
    ...
    >
      <span className="text-micro absolute left-3 top-2 text-muted-foreground/70">{d.label}</span>
    </div>
  )
})
