...
**Deferred-build backlog:** `BULLSEYE_BACKLOG.md` holds full specs for large deferred builds the owner will trigger with the word **"bullseye"** — the Freelance Delivery CRM (unified EMP/CLI/ASG/INV/TKT id nomenclature, AI-employee onboarding, assignment cards with client/payment/acceptance-criteria fields, Jarvis-as-ticket-dispatcher), agentic Client Onboarding + Invoicing (owner supplies repos/n8n to ingest), and Connection-Hub MCP connectors + npm-skill ingestion. Do NOT build these until the owner says "bullseye".

**Docs map:** this file (architecture + invariants) · `BULLSEYE_BACKLOG.md` (deferred specs, "bullseye"-gated) · `SETUP.md` (operator's usage guide) · `drizzle/` (versioned migrations, source of truth for the schema — auto-applied by `scripts/migrate.mjs` on every `pnpm build`/deploy; `setup.sql` is kept only as historical reference, don't re-run it) · `.env.example` (every env var, annotated).
